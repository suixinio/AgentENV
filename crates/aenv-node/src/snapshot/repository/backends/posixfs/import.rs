...
use super::artifacts::{run_artifact_blocking, PosixFsArtifactStore};
...
/// The full POSIX-filesystem artifact store: [`PosixFsArtifactStore`]'s delete
/// path plus the import path that needs overlaybd.
#[derive(Clone)]
pub struct PosixFsArtifactImporter {
    root: PathBuf,
    store: PosixFsArtifactStore,
}

impl PosixFsArtifactImporter {
    pub fn new(root: PathBuf) -> Self {
        Self {
            store: PosixFsArtifactStore::new(root.clone()),
            root,
        }
    }
...
#[async_trait]
impl SnapshotArtifactStore for PosixFsArtifactImporter {
    ...
    async fn delete_artifacts(
        ...
        self.store.delete_artifacts(id, publications).await
    }
}

#[derive(Debug)]
pub struct CollectedBuiltArtifacts {
...
mod tests {
    ...
    use super::PosixFsArtifactImporter;
    ...
    fn test_store(root: &std::path::Path) -> PosixFsArtifactImporter {
        PosixFsArtifactImporter::new(root.to_path_buf())
    }
