...
> 🔴 **已被后续变更取代（2026-08-29）**：`GET /sandboxes` / `GET /v2/sandboxes` 的只读豁免已从 `control_plane_gate::is_exempt` 删除——它自述的退役条件（删掉 gateway 的 `fetchNodeClusterList` 扇出）已经满足，`cluster_list.go` 不复存在。豁免表现在只剩 `/health`（任意方法）。因此本条探针里「无 token 的 `GET /sandboxes` ⇒ 非 403」这半句**不再是验收标准**，现行期望是 **403**；其余半句（无 token `POST` ⇒ 403、错误 token ⇒ 403、`/metrics` 结构性在 gate 之外）仍然有效。守卫见 `only_the_health_probe_is_exempt` 与 `the_sandbox_listing_is_gated_like_any_other_route`。
