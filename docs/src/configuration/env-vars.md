...
| `AENV_PAUSED_REGISTRY_BACKEND` | from config (`local`) | Select the cluster-wide paused-sandbox registry backend: `local` keeps a paused sandbox resumable only on the node that paused it, `postgres` records it in the shared registry so any node can resume it, `central` does the same through the scheduler instead of connecting to the database (needs `AENV_OBSERVABILITY_SCHEDULER_ENDPOINT`, not a DSN). Set it here rather than in the cluster ConfigMap: `deploy/k8s/run.sh` rewrites that ConfigMap from `config/default.toml` on every apply. |
| `AENV_PAUSED_REGISTRY_DSN` | unset | PostgreSQL DSN for the `postgres` registry backend. Required by that backend, which fails startup without it rather than falling back to `local`. Keep it in a Secret; it never belongs in the config file. |
