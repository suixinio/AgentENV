...
1. Provide Firecracker binary, kernel, tools drive, overlaybd runtime, and ublk daemon. These are automatically downloaded when the node server starts, or you can run `cargo run -p aenv-node --bin aenv-node -- --setup-only` to provision runtime dependencies independently.
...
12. Run `make test-uffd` for the `uvm-uffd` handler suite. It creates its own userfaultfd with `UFFD_USER_MODE_ONLY`, which an unprivileged account may do under `vm.unprivileged_userfaultfd=0`, so it needs no host setup and no capability runner. With `AENV_MEMORY_SNAPSHOT_BACKEND=uffd AGENTENV_MEMORY_SNAPSHOT_TRACK_DIRTY_PAGES=true` in the environment, the integration suites of step 8 resume every snapshot through the daemon's userfaultfd server instead of a memory device. Firecracker creates that userfaultfd itself: on a kernel with `/dev/userfaultfd` (6.1 and later) it opens that device and nothing else, so run `sudo scripts/tests/setup-uffd-access.sh <user> <group>` once to give the account read and write access (`aenv-node --setup-host` installs the same udev rule); on older kernels the syscall takes `CAP_SYS_PTRACE`, which `scripts/run-with-capabilities.sh` grants alongside `CAP_NET_ADMIN` and `CAP_SYS_ADMIN`. A node running as root needs neither; the daemon suite's `serve_memory_uffd_*` cases (`make test-ublk-daemon-nbd`) cover the daemon side without Firecracker by playing the VMM themselves.
...
6. Run setup against a clean or temporary Firecracker dependency directory so it
   ...
   ```bash
   cargo run -p aenv-node --bin aenv-node -- --config /path/to/config.toml --setup-only
   ```
