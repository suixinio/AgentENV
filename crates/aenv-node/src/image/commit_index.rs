...
pub const OVERLAYBD_COMMIT_FILE: &str = "overlaybd.commit";
...
pub fn digest_slug(digest: &str) -> String {
...
pub fn commit_file(commit_store: &Path, digest: &str) -> PathBuf {
    commit_dir(commit_store, digest).join(OVERLAYBD_COMMIT_FILE)
}

pub fn cached_commit_file_if_usable(
    commit_store: &Path,
    digest: &str,
    size: u64,
) -> Option<PathBuf> {
...
pub fn accept_existing_commit_file(destination: &Path, digest: &str, size: u64) -> Result<PathBuf> {
...
pub fn seed_commit_file_trusted_descriptor(
    commit_store: &Path,
    source: &Path,
    digest: &str,
    size: u64,
...
pub fn sanitize_filename_component(value: &str, max_len: usize) -> String {
