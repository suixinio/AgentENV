...
package is **deleted from the repository**, not merely superseded:
...
- `aenv-api` requires `[pg]` unconditionally now (there is no
  ...
  CLAUDE.md's snapshot-catalog section) — a prerequisite the Go-scheduler-based
  guide below never had.
...
`services/README.md` for the current architecture status. Everything past
this point is preserved for historical reference only, and describes the
deleted Go Scheduler's systemd deployment; do not follow it as literal
instructions.
