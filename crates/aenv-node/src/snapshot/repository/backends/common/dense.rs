...
pub async fn write_dense_overlaybd_layer_to_file(
    source: &Path,
    destination: &Path,
    ...
    let descriptor = aenv_core::digest::FileDigest::describe(destination)
        .await
        .with_context(|| {
            format!(
...
pub fn write_dense_overlaybd_layer_to_file_blocking(
    source: &Path,
    destination: &Path,
) -> Result<dense_export::DenseLayerDescriptor> {
