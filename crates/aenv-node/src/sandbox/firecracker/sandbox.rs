...
pub fn managed_snapshot_base() -> PathBuf {
...
impl FirecrackerCapturedSnapshot {
    ...
    pub fn in_caller_owned_dir(manifest: FirecrackerSnapshotManifest) -> Self {
        Self {
            manifest,
            _snapshot_root: None,
        }
...
impl FirecrackerSandbox {
    ...
    pub fn from_snapshot_config_with_override(
    ...
    pub async fn start_nowait(&mut self) -> Result<()> {
    ...
    pub fn host_interaction_ip(&self) -> Option<std::net::Ipv4Addr> {
        self.network_slot
            .as_ref()
    ...
    pub fn tools_drive_version(&self) -> &str {
...
impl FirecrackerSandbox {
    ...
    async fn start_fresh(&mut self, config: FirecrackerSandboxConfig) -> Result<()> {
        ...
        let (extra_drive_attachments, extra_drive_runtimes) =
            if config.common.extra_drives.is_empty() {
                ...
                let runtime_upper_mode = crate::sandbox::ublk::upper_mode_for(
    ...
    async fn prepare_snapshot_backing_drives(&mut self, extra_drives: &[ExtraDrive]) -> Result<()> {
        ...
        let runtime_upper_mode =
            crate::sandbox::ublk::upper_mode_for(ublk_config.overlaybd.runtime_upper_mode);
