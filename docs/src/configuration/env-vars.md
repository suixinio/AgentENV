...
| `AENV_SECRETS_LEGACY_BEARER_UNTIL` | empty | `[secrets].legacy_bearer_until`: an RFC 3339 instant after which the shared bearer above stops being accepted at the internal endpoints, leaving each broker's own projected ServiceAccount token as the only credential. Empty never closes the window — and while it is open, a caller presenting the bearer is scoped to no node, so the per-node bound on `POST /internal/credentials/resolve` does not apply to it. |
