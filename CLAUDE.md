...
- `make test-unit` runs `--lib --bins`. Tests in a `main.rs` or under `src/bin/` are part of daily validation, so put a test where it belongs, not where the runner can see it.
...
- **`aenv-api`** (deciding half, multi-replica): the full REST surface, the in-process node registry (`src/node_registry/`) that answers the `Scheduler` gRPC contract (`services/api/proto/scheduler.proto`) — the only implementation of it — with round-robin placement (`RoundRobinStrategy`; the `strategy=round_robin` metric label is a series identity and must not change shape), the Redis sandbox-to-node binding store (`src/binding_store/`, `AENV_BINDING_STORE_REDIS_URL`), the PostgreSQL snapshot catalog and the PostgreSQL paused-sandbox registry (`crates/aenv-api/src/orchestrator/paused_registry/postgres/`, selected by `[orchestrator.paused_registry].backend = "postgres"`; `"local"` is still supported). It **requires `[pg]`** and refuses to assemble without it. Node discovery: `[cluster].node_discovery_mode` = `"kubernetes"` (default; watches EndpointSlices per `[cluster.kubernetes_discovery]`) or `"static"` (`[cluster].static_discovery_nodes`). Rolling this half back is an image digest change on its Deployment, no flags; see `services/README.md`'s "Rolling the API half back".
- **`aenv-node`** (running half, DaemonSet): holds no catalog — its `SnapshotRepository` carries `NoSnapshotCatalog`, which refuses catalog calls; a node stages bytes (`SnapshotRepository::stage`) and `aenv-api`'s `commit_staged` writes the row. Answers the user-facing REST surface with 404 (`src/api/role_gate.rs`); its real create/build path is gRPC (`crates/aenv-node/src/node_server/service.rs`). Heartbeats to `[cluster].scheduler_endpoint` (always `aenv-api`), hot-reloadable via `src/scheduler_endpoint.rs::SchedulerEndpointSource`. For any `[orchestrator.paused_registry].backend` other than `"local"` it warns and wires a no-op registry.
- **`gateway`** (`services/gateway/`): reverse proxy. All user-facing REST goes to `gateway.rest_upstream_addr` (required non-empty); `GET /nodes*` and `GET /registry/sandboxes` are aggregated from Scheduler RPCs; sandbox data-plane traffic (routing headers or a proxy domain) is routed by `LookupNode` against `gateway.scheduler_addr`, bounded by `GATEWAY_COLD_LOOKUP_TIMEOUT`.
...
Removed switches: `--role`/`AENV_ROLE` are refused by argument parsing; `AENV_NODE_PLACEMENT_SOURCE`, `AENV_BINDING_STORE_BACKEND`, `AENV_SNAPSHOT_CATALOG_{WRITE,READ,MIRROR_PATH}` and the gateway's query-only/fallback variables are ignored. `docs/src/configuration/env-vars.md` lists each with what setting it does today. `services/shared/config/snapshot_catalog_manifest_test.go` walks `deploy/k8s/base` and fails if any workload still declares a catalog variable — keep it working.
...
- **Orchestrator** (`src/orchestrator/`): sandbox lifecycle state machine (Creating → Running → Forking | Snapshotting | Pausing → Paused, Resuming, Killing) in `service.rs`. `fork_sandbox` forks one running sandbox into N children on the same node; `capture_snapshot` drives Running → Snapshotting → Running, rolling back on recoverable failure and tearing down on `SandboxCaptureError::Terminal`. Keeps `OrchestratorCounters`, derives running counts and allocated resources from metadata (`aggregate_resource_metrics`), publishes best-effort broadcast lifecycle events, evicts expired sandboxes, pauses and persists running sandboxes on graceful shutdown (`SandboxPersister`) and restores them as `Paused` on startup.
- **Observability** (`src/observability/`): node snapshots for admin/node APIs; `reporter.rs` sends periodic `Heartbeat` RPCs and drains lifecycle events into `ReportSandboxEvent`; the heartbeat reply names paused sandboxes the control plane no longer records here, and the node discards each after a second consecutive naming. `machine.rs` optionally runs `cpu-template-helper template dump`; the registry ANDs every node's config (`src/node_registry/cpu_template.rs`, golden-tested) and returns the intersection, applied to new VMs via Firecracker's pre-boot `PUT /cpu-config`.
...
- `crates/aenv-api`: the deciding half — everything that talks to PostgreSQL (`pg`, paused-registry backend, snapshot catalog backend). Ships `aenv-api` and `aenv-snapshot-image`
