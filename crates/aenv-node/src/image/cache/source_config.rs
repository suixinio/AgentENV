...
impl SourceImageCache {
    pub async fn acquire_lock(self: &Arc<Self>, image_config_path: &Path) -> SourceImageLock {
...
pub struct ImageCacheSourceImageKey {
...
impl ImageCacheSourceImageKey {
    ...
    pub fn repository_scope(&self) -> Option<&str> {
        self.repository_scope.as_deref()
    }
}

pub async fn read_source_image_metadata(
    metadata_path: &Path,
...
pub fn source_image_config_filename(key: &ImageCacheSourceImageKey) -> String {
...
pub fn source_image_metadata_path_for_config_path(path: &Path) -> PathBuf {
...
pub fn source_image_config_is_usable(path: &Path) -> bool {
...
pub async fn write_json_atomic<T: Serialize>(path: &Path, value: &T, label: &str) -> Result<()> {
    let parent = path
        .parent()
