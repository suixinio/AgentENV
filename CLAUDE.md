...
- `crates/aenv-api`: the deciding half — everything that talks to PostgreSQL (`pg`, snapshot catalog backend, `secret_refs`), the Vault secrets backend. Ships `aenv-api` and `aenv-snapshot-image`
