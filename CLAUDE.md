...
make check-crate-boundaries   # aenv-api links no overlaybd/ublk/iroh/envd; aenv-node no sqlx; aenv-core neither runtime
...
- **Observability** (`src/observability/`): node snapshots for admin/node APIs; `crates/aenv-node/src/observability/reporter.rs` sends periodic `Heartbeat` RPCs, drains lifecycle events into `ReportSandboxEvent`, and reaches shutdown through `server_main`'s `HeartbeatReporter`. `machine.rs` optionally runs `cpu-template-helper template dump`; the registry ANDs every node's config (`src/node_registry/cpu_template.rs`, golden-tested) and returns the intersection, applied to new VMs via Firecracker's pre-boot `PUT /cpu-config`.
...
- `aenv-core` (root, lib `aenv_core`): everything both halves need — HTTP API surface, orchestrator, snapshot catalog model, cfg, node registry, binding store, node client
...
- `crates/aenv-api`: the deciding half — everything that talks to PostgreSQL (`pg`, snapshot catalog backend, `secret_refs`/`secret_values`) and the broker's resolve endpoint. Ships `aenv-api` and `aenv-snapshot-image`
