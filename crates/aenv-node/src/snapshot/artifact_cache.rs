...
pub struct CacheArtifactLease {
    pub _handles: Vec<CacheHandle>,
}
...
impl LocalArtifactCache {
    pub fn new(cache_root: PathBuf, max_size_gb: Option<u64>) -> Result<Arc<Self>> {
    ...
    pub async fn ensure_cached<F, Fut>(self: &Arc<Self>, key: &str, fetch: F) -> Result<CacheHandle>
    where
        F: Fn(PathBuf) -> Fut,
    ...
    /// share a path.
    pub async fn ensure_cached_at<F, Fut>(
        self: &Arc<Self>,
        key: &str,
        local_path: PathBuf,
        fetch: F,
