...
| ~~`AENV_EGRESS_CA_ROOT_CERT_PATH`~~, ~~`AENV_EGRESS_CA_ROOT_KEY_PATH`~~ | — | **Removed.** They declared `[egress_ca]`, the root this half signed a seven-day name-constrained intermediate per node from, at `POST /internal/egress/intermediate`. That layer defended only a compromised node, which the threat model excludes — values never enter a guest, and a node may hold credential values — so each broker now mounts the root and signs leaves with it directly (`docs/proposals/2026-09-07-egress-ca-single-layer.md`). The `[egress_ca]` section, the endpoint, `crates/aenv-api/src/egress_ca/` and the api Deployment's `egress-ca` mount were deleted together. **Setting them today does nothing:** no field declares the names, so they are read by nothing and refused by nothing. `services/shared/config/egress_ca_manifest_test.go` fails if the api half mounts the Secret again. |
...
| `AENV_MEMORY_SNAPSHOT_BACKEND` | `block` | Override `[memory_snapshot].backend`: `block` restores memory through a read-only device, `uffd` has the ublk daemon answer Firecracker's page faults over a Unix socket. `uffd` needs `[memory_snapshot].track_dirty_pages = true` only when `[memory_snapshot.uffd].write_protect = false`. |
...
| `AENV_MEMORY_SNAPSHOT_HUGE_PAGES` | `false` | Override `[memory_snapshot].huge_pages`: boot new VMs with guest memory on 2 MiB hugetlbfs pages. Requires `backend = "uffd"`; the snapshot records the setting and restores only over userfaultfd. |
...
| `AENV_MEMORY_SNAPSHOT_UFFD_WRITE_PROTECT` | `true` | Override `[memory_snapshot.uffd].write_protect`: take dirty pages from userfaultfd write protection instead of the KVM dirty log. `false` requires `track_dirty_pages = true`. Read only when `backend = "uffd"`. |
...
| ~~`AENV_EGRESS_CA_ISSUER_URL`~~, ~~`AENV_EGRESS_CA_ISSUER_TOKEN_FILE`~~ | — | **Removed.** They asked the api half for this node's egress intermediate and authenticated the ask with this Pod's projected token. There is no intermediate: a broker signs leaves with the root it mounts (`docs/proposals/2026-09-07-egress-ca-single-layer.md`). **Setting them today does nothing:** no field declares the names, and `issuer_url` or `issuer_token_file` under `[ca]` in `aenv-egress.toml` is ignored the same way. |
| `AENV_EGRESS_CA_CERT_PATH` | from file | `ca.cert_path`: the root that signs every intercepted-name leaf, and the certificate each node hands its guests as `[egress_broker].guest_ca_cert_path`. Required. |
| `AENV_EGRESS_CA_KEY_PATH` | from file | `ca.key_path`: that root's private key, mounted by this DaemonSet alone. Required — a broker with only one half of the pair refuses to start. |
