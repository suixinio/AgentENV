...
| Artifact | Location | Contents | Purpose | Lifecycle |
| --- | --- | --- | --- | --- |
| Paused sandbox records | `<persisted_sandbox_store_path>/records/{sandbox_id}.json` | One compact JSON record per sandbox containing version, lifecycle, sandbox metadata, artifact root, and backend state | Restores paused sandboxes after server restart | Record written after pause succeeds. Marked `resuming` before resume. Rolled back on resume failure. Deleted on resume/delete. |
