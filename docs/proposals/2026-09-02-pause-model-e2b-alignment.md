...
- **节点拒绝 create 时换节点重放（对齐 e2b `PlaceSandbox`）。** api 半边的放置视图异步刷新，origin 刚置 draining 的几秒内 `preferred_node_id` 仍命中它，节点以 `node is isolated` 拒绝；pve-mf 首轮实测 1 秒内 500、7 秒后才 201。现在 `RemoteSandboxBackend::start` 把以 Unavailable / FailedPrecondition / ResourceExhausted 拒绝的节点（以及连不上的节点）加入 `NewSandboxHint.excluded_node_ids`，撤销占位后重新放置，最多三次；被排除的节点即使是偏好节点也不再被选。请求本身的错误（InvalidArgument、NotFound、Internal）不重试。
