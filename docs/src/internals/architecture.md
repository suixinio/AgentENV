...
`[memory_snapshot].backend` selects one of two restore paths over the same stacked memory overlaybd layers.

Where the layers come from is decided before either path runs. With the `oss` repository, every layer a pause uploads is also hard-linked into the image cache's commit store (`OverlaybdLayerStore::adopt_layer`), and overlaybd opens a lower from `<dir>/overlaybd.commit` before it goes to object storage, so a resume on the node that paused reads the new layer locally. That copy is cache, reclaimed like any other; a node without it reads through overlaybd's full-file cache, whose entries are keyed by layer digest and persist across resumes, so a layer is fetched once per node. With `track_dirty_pages` on, a pause layer holds only the pages written since the last resume and the rest of the working set stays in parent layers those caches already hold.

**`block`** (default): a read-only device (ublk or nbd, per `[ublk].transport`) is created from the layers and passed to Firecracker as a `BackendType::File` memory backend. Firecracker mmaps the block device and COWs pages into anonymous memory on first write, so the underlying device is never modified. Multiple sandboxes booting from the same snapshot share a single memory device via reference counting, so the Linux page cache holds each clean page once for all of them. Every guest page fault on a not-yet-resident page is one 4 KiB read through the block layer and the transport.

**`uffd`**: no device exists. The node asks the daemon (`ServeMemoryUffd`) to bind a socket under the sandbox's Firecracker work dir, loads the snapshot with a `Uffd` memory backend pointing at it, and Firecracker hands the daemon its userfaultfd and region mappings. The daemon's `uvm-uffd` handler (one thread per VM, its own ring) reads each faulting page straight from the opened image and installs it with `UFFDIO_COPY`; all-zero pages, pages past the image and pages the guest discarded (`UFFD_EVENT_REMOVE`) go in as the zero page. Faults on one page are deduped and installs are bounded by `[memory_snapshot.uffd].max_inflight`. The first resume of a template snapshot records the pages it installed as `{home_path}/mem-prefetch/{snapshot_id}.json`, keyed by the image's page size and byte size; every later resume of that snapshot replays the list in the background on a quarter of the fault budget, so the working set is present before the guest asks for it. With the `oss` repository the list travels with the snapshot (`mem_prefetch.json` under its prefix): a resume on a node without the list downloads it, and a node whose list is not yet there uploads it, so the fleet replays the first list any node recorded. A sandbox's pause row is resumed at most once, so it records nothing. Pages are private anonymous memory per VM, so nothing is shared between sandboxes of one snapshot. A page read that keeps failing past `read_retry_secs` ends the handler and the guest is unbacked rather than wedged: every later fault gets a plain zero page, so the node polls the server every few seconds and kills the Firecracker process the moment the daemon reports an exit (or stops answering), the same loud end a block device gives with an I/O error. `StopMemoryUffd` closes the descriptor on sandbox stop. The daemon's `/metrics` carries `uffd_memory_servers`, `uffd_memory_server_exits_total{outcome}` and the per-server counters summed at stop (`uffd_memory_faults_total` and siblings). The backend requires `track_dirty_pages`, because every installed page is anonymous and the `mincore` readout would otherwise capture the whole faulted working set on pause; `track_dirty_pages` is refused under PVM, so `uffd` is a KVM-only choice today. With `[memory_snapshot].huge_pages = true` new VMs boot with guest memory on 2 MiB hugetlbfs pages: one fault fills 2 MiB, the handler scales its install slots down so the bytes in flight stay near the 4 KiB figure, and the zero page is a 2 MiB `UFFDIO_COPY` from a zero buffer because hugetlb has no shared zero page. The setting is fixed at boot and travels in the snapshot (`CommittedSnapshot::huge_pages`), which therefore restores only through this backend and only on a host that can hand out 2 MiB pages; while `track_dirty_pages` is on, KVM maps the guest at 4 KiB anyway, so the fault-count reduction is the benefit and the TLB one waits for write-protect based dirty tracking. `docs/proposals/2026-09-10-memory-uffd-backend.md` records the measured trade-off and the next step (write-protect based dirty pages).
...
| Orchestrator | `crates/aenv-node/src/orchestrator/`, over the model in `src/orchestrator/` | Sandbox lifecycle state machine (Creating, Running, Forking, Snapshotting, Pausing, Killing), auto-eviction, incremental runtime metrics; a pause ends in a snapshot-catalog row and no record. Its collaborators -- the store, the backend factory, the pause publisher, the grant issuer -- all arrive at construction. Callers reach it through `SandboxOrchestration`, which both halves answer, or `NodeOrchestration`, the subtrait of nine methods only a process holding VM handles can answer |
