...
| `AENV_PAUSED_REGISTRY_BACKEND` | from config (`local`) | Select the cluster-wide paused-sandbox registry backend: `local` keeps a paused sandbox resumable only on the node that paused it, `postgres` records it in the shared registry so any node can resume it. Set it here rather than in the cluster ConfigMap: `deploy/k8s/run.sh` rewrites that ConfigMap from `config/default.toml` on every apply. |
