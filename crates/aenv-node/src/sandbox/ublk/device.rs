...
impl UblkDeviceManager {
    ...
    pub async fn notify_sandbox_ready(&self, device_key: &Path) {
    ...
    pub async fn release_device(&self, device: &UblkDevice) -> Result<()> {
    ...
    pub async fn restack_snapshot_device(
        &self,
        device: &UblkDevice,
    ...
    /// handle is dropped.
    pub async fn get_or_create_shared_mem(
        &self,
        spec: &UblkCreateSpec,
        virtual_size: u64,
