...
use aenv_node::snapshot::repository::{
...
mod dual_write {
    ...
    use super::*;
    use aenv_node::snapshot::repository::backends::storage::{
        PosixFsBackend, PosixFsBackendConfig,
    };
    use aenv_node::snapshot::repository::mirror::{
        CentralCatalogWrites, DualWriteCatalog, MirrorBacklog, MirrorDirection, MirrorTargets,
    };
    use aenv_node::snapshot::repository::RepositoryResult;
    ...
    async fn both(central: Arc<CentralSnapshotCatalog>) -> Both {
        aenv_node::cfg::ConfigManager::init_global().expect("a config should load");
        let workspace = tempfile::TempDir::new().expect("tempdir should exist");
    ...
    async fn creating_and_failing_a_template_reaches_both_catalogs() {
        ...
        assert!(matches!(
            central_row.source,
            SnapshotSource::Template { ref build }
                if build.status == aenv_node::snapshot::TemplateBuildStatus::Error
        ));
        let object_store_row = both
            .object_store
            .get(&id.to_string())
        ...
        for (which, row) in [
            ...
            assert!(
                matches!(
                    row.source,
                    SnapshotSource::Template { ref build }
                        if build.status == aenv_node::snapshot::TemplateBuildStatus::Building
                ),
    ...
    async fn a_queued_history_makes_the_hijack_impossible_rather_than_undone() {
        ...
        let queued = both
            ...
            .queue_history_toward_central(both.object_store.as_ref()
                as &dyn aenv_node::snapshot::repository::interfaces::SnapshotCatalog)
            .await
            .expect("the history should queue");
...
mod bytes_then_commit {
    ...
    use aenv_node::snapshot::repository::{SnapshotRepository, StagedSnapshot};
    use aenv_node::snapshot::{SnapshotManager, SnapshotPublishMetadata};
    ...
    async fn bytes_land_here_and_the_commit_happens_in_another_process() {
        ...
        let (_, _, manifest) =
            aenv_node::snapshot::mock::write_mock_built_artifacts(artifacts.path())
                .expect("mock artifacts should write");
...
async fn the_read_side_gates_census_counts_rows_the_resolvable_reading_hides() {
    use aenv_node::snapshot::repository::mirror::CatalogCensus;
