...
🔴 阶段四 folds this Go control plane into the Rust `--role api` binary one RPC group at a time (`docs/proposals/2026-08-20-service-decomposition.md`'s phase four section): `src/node_registry/` ports node discovery, heartbeat receipt (including the CPU-config intersection, `src/node_registry/cpu_template.rs`), and P2P peer/artifact lookup; `src/orchestrator/paused_registry/postgres/` ports the paused-sandbox registry onto the same shared `[pg]` pool the snapshot catalog uses. `[cluster].node_placement_source = "native"` switches `--role api` onto its own in-process registry instead of dialling the scheduler for placement/heartbeat/paused-registry RPCs, and `deploy/k8s/`'s base layer runs that way by default as of 阶段四 E1-E3 — the scheduler Deployment/Service/PDB are commented out of `kustomization.yaml`'s `resources:` (not deleted: `services/scheduler`'s Go source, and one release image tag, stay in the tree as the rollback target) and every runtime reference that used to dial `agentenv-scheduler` (node heartbeats, api's own resume placement, the gateway's `scheduler_addr`) now dials `agentenv-api` instead. The one gap this fold has not closed is `ListRegistrySandboxes` — `NodeRegistryGrpcService::list_registry_sandboxes` in `src/node_registry/grpc_service.rs` still answers `Unimplemented`, so the gateway's registry-listing debug endpoint (`services/gateway/internal/registry_list.go`) is the one caller still without an api-side answer. See `services/README.md` for the current deploy status and rollback steps.
