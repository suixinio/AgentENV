...
| `AENV_NODE_PLACEMENT_SOURCE` | `scheduler` | Where `--role api` resolves a known node's current address, cluster membership, and sandbox placement/routing: `scheduler` (default, unchanged behavior — asks the scheduler over gRPC for all of it, and requires `AENV_OBSERVABILITY_SCHEDULER_ENDPOINT`) or `native` (answers all of it from api's own process instead — a local `src/node_registry` node registry fed by Kubernetes discovery and node heartbeats for node identity/membership, and `AENV_BINDING_STORE_BACKEND`'s routing table for `Schedule`/`LookupNode`/`RecordAssignment`; no scheduler endpoint is required in this mode). |
| `AENV_BINDING_STORE_BACKEND` | `in_memory` | Which sandbox-to-node routing table backend `Schedule`/`LookupNode`/`RecordAssignment` answer from under `AENV_NODE_PLACEMENT_SOURCE=native`: `in_memory` (one replica's own table — refused at startup, since `--role api` runs more than one replica) or `redis` (the shared table every replica reads and writes). Unused under `scheduler` mode. |
...
| `AENV_PAUSED_REGISTRY_BACKEND` | from config (`local`) | Select the cluster-wide paused-sandbox registry backend: `local` keeps a paused sandbox resumable only on the node that paused it; `central` records it in the shared registry, reached over gRPC at `AENV_OBSERVABILITY_SCHEDULER_ENDPOINT` — this is `--role node`'s only cluster-wide option, and the one the DaemonSet still runs (`--role node` never holds a `[pg]` DSN); `postgres` connects directly to the registry database over the shared `[pg]` pool instead of dialling anything, folding the scheduler's own registry service into this process — legal only under `--role api`/`--role all` (`--role api` additionally requires `[cluster].node_placement_source = "native"` for a lease-renewal roster; see CLAUDE.md's paused-registry note), and this is what `deploy/k8s/base/agentenv-api-deployment.yaml` runs by default since 阶段四. Set it here rather than in the cluster ConfigMap: `deploy/k8s/run.sh` rewrites that ConfigMap from `config/default.toml` on every apply. |
