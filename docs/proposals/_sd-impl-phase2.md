...
>
> #### 🔴 再补一条：补回来的那句不变式，比它自己声称的弱（2026-08-21，2b 验收之后修）
>
> 上一条把「lag == 0 必须意味着两边一致」写成了目标，但**落法只保证了「两边有同一批 id」，
> 没保证「同一批行里装的是同一批内容」**。集群实测：回填把对象存储里的 32 行原样重放进 PG 时，
> 每一行都是**一次新的写**，于是 `created_at_ms` 记的是**回填那一刻**——
> PG 32 行全读 `2026-08-21 07:10:13`，对象存储那边是 `2026-08-20 22:06–22:08`。
> 列表查询是 `ORDER BY s.created_at_ms DESC, s.id ASC`，所以：
> ① PG 返回的顺序与对象存储**完全不同**；② 从这一列服务出去的 `createdAt`
> **对每一条翻开关之前的快照都是错的**。而 `lag == 0 && diverged == 0` 照旧报「一致」。
>
> 🔧 **已修（B-1）**，两件事：
>
> 1. **时刻随 `SnapshotCommit` 走。** 新增 `created_at_unix_ms: Option<i64>`，在**暂存字节的地方
>    读一次钟**（`composite.rs::stage`，与 `staged_at_unix_ms` 同一个值），两个目录都写它。
>    回填用 `commit_from_record` 把**记录原有的**时刻抄进去，而不是让重放盖上自己的钟。
>    顺带修掉了同一个缺陷的小号版本：活路径上两个目录各自 `now()`，
>    **每一次 pause 的两行都差一个 RPC 的延迟**，任何逐行比对都不可能相等。
>    `None` 只留给「本字段出现之前写下的队列条目」，语义是「哪个存储接住就由它定」。
> 2. **一致性检查真的去比内容。** 一条待补条目在离开队列之前，
>    `drain_once` 把**两个目录各读一遍并逐字段比对**；不一致就**继续算欠账**（`Retry`），
>    不让 lag 归零；一直不一致就走重试上限落成 divergence，理由里点名**是哪个字段**。
>    新增计数器 `agentenv_snapshot_catalog_mirror_content_mismatch_total{direction,field}`。
>
> **比什么**：`created_at`、`alias`、`resources`、`source`（种类 ＋ 源沙箱 id）、
> 模板构建 `status`、以及 `committed` 整个载荷（按结构比，不按序列化字节——
> `CommandContext` 里两个 `HashMap` 的 JSON 键序每进程都不一样）。
>
> **不比什么，以及为什么**（每一条都是**正常路径上就会不同**的字段，比了会让健康集群永久报分歧）：
> `updated_at_ms`（两边各自的钟；PG 侧调用方不给值时还有触发器补）、
> `build.started_at_ms` / `finished_at_ms`（本版本根本没往中心目录发过）、
> `build.error_reason`（回填给「双写之前就已失败、而对象存储没记原因」的构建**合成**了一个理由，
> 这是有意的不对称）。
>
> **还比不了什么** —— 🔴 这句必须原样传给 2c：
> **只有进过队列的行会被比。** 活路径上两边都写成功的写从不入队，因此从不被比对。
> ⇒ `lag == 0 && diverged == 0` 的准确含义是
> **「曾经欠下的每一笔都重放过、并且当时比对一致」**，
> **不是**「此刻两个目录相等」。2c 的判据不能只读这两个 gauge（下一节）。
>
> #### 🔴 分歧记录会比它所描述的快照活得更久（2026-08-21，同轮修）
>
> `clear_divergences` 挂在 delete 上，但**delete 是 gateway 路由的**，而分歧记录是**节点本地**的。
> 集群实测：13 次删除按 **6 / 8** 落在两台节点上，于是一条分歧记在 worker-01、
> 而那条快照是从 master-01 删掉的 —— worker-01 的 `mirror_diverged{direction="central"}`
> **永久停在 1**，描述的是一条两个目录都已经没有的快照，**任何 API 调用都清不掉**。
> 那个 gauge 正是 2c 的放行判据 ⇒ 一条滞留记录就能把切换永久卡死，
> 运维手上除了「把这台节点的 mirror store 删掉」没有别的办法。
>
> 🔧 **已修（B-2）**：补偿器每 10 个 pass 扫一遍自己的分歧记录，
> **两个目录都已经不持有那条快照时把记录退休**（`retire_settled_divergences`），
> 计数器 `agentenv_snapshot_catalog_mirror_divergences_retired_total{direction}`。
> 选它而不是「广播 delete」或「把记录搬到集群里」，是因为它**不需要今天不存在的东西**，
> 而且它问的正是 delete 路径本来就在回答的那个问题——
> 「两个目录都不再持有它」——只是直接去问，而不是靠「我看见 delete 经过」去推。
>
> **它为什么不会退休一条仍然成立的分歧**，两条守则：
> ① **两个目录都必须答，且都必须答「没有」**。任一边还持有 ⇒ 保留；任一边**答不上来** ⇒ 也保留，
> 「我没看成」不等于「它没了」。② **队列里还欠着这条快照的写时整条跳过** ——
> 否则一次重放可能在扫描刚退休掉记录之后，把行重新写回其中一个目录，
> 变成扫描亲手复活了自己刚刚退休的那条分歧。
> 每轮最多看 64 条、并从上一轮停下的地方接着看：每条要读**两个**存储，
> 其中一个正是整个迁移的验收数字所依赖的那个。

> #### 🔴 一行已经写下的 `created_at_ms` 修不回来 —— 以及"清库"必须连 mirror store 一起清
>
> **结构性事实**：目录里**没有任何一条语句会改一行已存在的 `created_at_ms`** ——
> `insertSnapshotSQL` 是 `ON CONFLICT (id) DO NOTHING`，`commitSnapshotSQL` 只写
> status / payload / published / origin / updated_at / 资源 / execution id。
> 这是**有意的**（创建时刻本就该不可变），但连带结论是：
> **一行用旧代码写坏了时刻的行，只能删掉重建，不能就地修**。
> 镜像队列重放的是**操作**，不做行级对账，所以它也不会去修。
>
> 🔧 **本集群已不受影响**：PG 已被清空回 0 行（2026-08-21），那 32 行不复存在。
> 这条留下来，是因为它对**任何跑过旧代码又保留着行的部署**仍然成立。
>
> 🔴 **但清空 PG 引入了一个新的、同形的陷阱，下一次开双写之前必须先处理：**
> 回填那个一次性标记（`meta:history_queued`）在**节点本地的 mirror store**里
> （`snapshot.catalog.mirror_backlog_path`，默认 `$AENV_HOME/snapshot-catalog-mirror`），
> **清 PG 完全不会碰它**。⇒ 下次把 `write` 打回 `both`：
> `queue_history_toward_central` 看见标记直接 `return Ok(0)`，
> **32 条历史一条都不会重新入队**；`mirror_lag{central}` 与 `mirror_diverged{central}` 双双读 0；
> `guard_read_side` 的历史条件也**通过**（标记确实在盘上）——
> **PG 0 行、对象存储 32 行、所有数字说一致**，正是 F-1 那个失效原样重现。
>
> 新加的内容比对在这里**也帮不上忙**：没有条目入队，就没有东西被比。
> ⇒ **两条动作，缺一不可**：
> ① 重开双写之前**清掉两台节点的 mirror store**（连标记带队列），让回填重跑一遍；
> ② 放行判据按上面「登记、不修」①，用 **PG 行数 vs 对象存储记录条数的直接比对** ——
> 它是这一族失效里唯一一个**不依赖任何标记、任何 gauge**的检查，
> 这次也正好是它能抓住而 gauge 抓不住的。

#### 🔴 登记、不修：2c 出场判据的三个设计开放项（2026-08-21）

**① 翻开关那一刻的 gauge 是观测不到的 ⇒ 判据不能建在它上面。**
回填在**约 100 毫秒**内跑完（一台 `queued=32` → `repaired=32` 用了 +126ms，另一台 +93ms）。
⇒ 在一个健康集群上，`mirror_lag{direction="central"}` **根本抓不到非零的那一瞬**；
而它读 0 的时候，「回填跑完了」和「回填从来没跑」是同一个读数。
🔴 **这是团队负责人的决定，照此登记**：
**2c 必须以一次直接比对放行 —— PG 行数 vs 对象存储记录条数 ——
并以 `agentenv_snapshot_catalog_mirror_repaired_total{direction="central"}` 佐证；
不许以那个 gauge 放行。**

**② 读侧那道闸，在本版本的任何合法配置下都执行不到。**
`AppConfig::validate_snapshot_catalog`（`src/cfg.rs:1243`）在 `MirrorBacklog::guard_read_side`
**上面一层**就把 `read = "postgres"` 拒了（`(Both, Postgres)` 与 `(Postgres, Postgres)` 都 bail）。
⇒ `01dd072` 往 `guard_read_side` 里加的全部东西 —— 包括那条「历史标记没落盘就不许切」的拒绝 ——
**除了单元测试之外从来没有执行过**。
🔴 **2c 里打开 `(both, postgres)` 那条 arm 的人，继承的是一道"生产环境从未跑过"的闸，
必须专门去把它跑一遍**（三个拒绝分支各一次），而不是假定它工作。

**③ 每台节点各自回填一整份历史。** 本轮是每台 32 条重放；规模上是 **N × 节点数**。
一台新加入的节点会把另一台已经镜像过的历史重新枚举一遍、
重放进一个已经同意的目录里（第一趟就全部 repaired）。
代价是真实的，不是疏忽 —— 替代方案是一个集群级的「这件事已经做过」的事实，
而本阶段没有任何地方能放它。

**④ `try_start_build` 那一类分歧，在这个集群上从来没出现过** —— 整轮**没有跑过任何模板构建**。
§9.2 上面那条「每一个模板的 PG 行都停在 `waiting`」是**从代码推出来的**，
不是从这台集群上量出来的。2c 的对照面要自己造一次。
...
2. `catalog_mirror_lag` 连续 7 天为 0；
   🔴 **经 §9.2 两次修订之后这条已经不够**：还要 `mirror_diverged{direction="central"}` 同时为 0，
   而**这两个 gauge 都不构成 2c 的放行判据** —— 放行判据是**PG 行数 vs 对象存储记录条数的直接比对**，
   见 §9.2「登记、不修」①。这里保留原文，是因为 §9.3 讲的是**拆掉双写**（阶段 3 之后），
   与 2c 的**读切换**不是同一道闸。
...
### 13.4 🔴 一个没查清的观测：播 30 条快照打出 60 次 `get/catalog/error`（🔧 当晚已查清并修掉）
...
🔧 **已查清并修掉（同日晚，提交 `4653374`）—— 上面的现象与判断都成立，原因不是猜的那一个。**

| 项 | 结论 |
|---|---|
| 那 60 次是什么 | 每次创建两发**合法落空**的别名查询：`create()` 与 `bind_alias()` 各经 `load_alias_target` 打一次 `GET catalog/aliases/{alias}.json` ⇒ 404。**不是 `exists`**（上面那句猜测猜错了对象，但猜对了性质：确实是"存在性查询被记成了错误"）|
| 为什么会记成 error | OSS 后端的每条目录读路径都把同一个 `NotFound` 收敛成 `Ok(None)`，而计数器**只有 `ok` / `error` 两个取值**可说 ⇒ 落空只能落在 `error` 上 |
| 修法 | `outcome` 增加一个取值：**`ok` / `not_found` / `error`**（`src/snapshot/repository/metrics.rs:114-116`）。`not_found` ＝ 请求完成、存储答"没有这个对象"；`error` 只剩它字面的意思。HEAD 答"不在"、DELETE 没东西可删，归同一类 |
| 顺带 | POSIX 侧同样的查询**本来一次都没记**（`exists()` 守卫在被埋点的读之前就返回了）⇒ 改成直接读并处理 `ENOENT`，两个后端对"一次查询要花多少"的说法这才一致 |

🔴 **对本节基线的影响，两条**：

1. **`1 + N` 的形状不变。** §13.1 的所有数字仍然有效 —— 变的是**落空**怎么标，不是列表怎么读。
2. 🔴 **但标签集变了。** §13.1 是在 `sd2a-7cf9c30` 上量的，那一版只有 `ok` / `error`；
   **在修过之后的镜像上重跑 P0，同一批落空会出现在 `outcome="not_found"` 上**。
   ⇒ 拿本节的数字做 2c 对照时，先确认两侧跑的是不是同一版标签集。
   上面那条"别对 `outcome="error"` 告警"的忠告，射程是 `sd2a-7cf9c30` 这一版；
   修过之后 `error` 才真的只剩"答不上来"。
...
| **C3** | §10.1「全仓没有对象存储请求计数指标 ⇒ 今天没法观测」＋ 那八个 `client.rs` 行号 | 指标**已经埋好并上线**（这正是 2a 的 Rust 侧交付物）⇒ 那句"今天没法观测"只作为**动手前的现状**存档。🔧 **八个行号全部漂了**（埋点本身把文件撑开了），现值：`get_bytes:117` / `get_to_file:135` / `exists:163` / `list_keys_recursive:182` / `put_bytes:213` / `put_file:251` / `delete:305` / `delete_prefix:325`（**在 `sd2a-7cf9c30` 上**）。🔧 同日提交 `4653374` 之后又漂了五个：`list_keys_recursive:188` / `put_bytes:221` / `put_file:259` / `delete:313` / `delete_prefix:339`；前三个（`get_bytes` / `get_to_file` / `exists`）未动 |
