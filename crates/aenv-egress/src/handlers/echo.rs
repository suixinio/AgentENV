...
/// What the `echo` handler writes as its first line: the identity the broker
...
/// opens no upstream and reads no credential; it exists so a test can prove
/// identity end to end, and both broker modes register it so neither one has
/// a handler set the other lacks.
pub struct IdentityEchoHandler;

impl IdentityEchoHandler {
    pub const NAME: &'static str = "echo";
}

#[async_trait]
impl Handler for IdentityEchoHandler {
    fn name(&self) -> &str {
        Self::NAME
    }
...
mod tests {
    ...
    async fn the_embedded_transport_returns_the_identity_and_echoes_without_a_socket() {
        ...
        header.handler = IdentityEchoHandler::NAME.into();
