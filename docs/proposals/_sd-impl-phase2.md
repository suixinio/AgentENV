...
> #### 🔴 修的是"不再发生"，不是"已经写坏的那 32 行"
>
> 🔴 **集群上 PG 里那 32 行的 `created_at_ms` 仍然是错的，本批修不了它们。**
> 目录里**没有任何一条语句会改一行已存在的 `created_at_ms`** ——
...
> 而且两台节点的 mirror store 里那个一次性的历史标记**已经落盘**，
> 所以重启也不会再跑一次回填。
>
> 新加的内容比对**能查出来**这件事（回填条目重放后逐行比 ⇒ `created_at` 不等 ⇒ 继续算欠账 ⇒
> 到上限落成 divergence ⇒ 判据拒绝放行），但那需要**先让这些条目重新入队**。
> ⇒ **2c 动手之前的清场动作**：把这个 cluster 的目录行清掉
> （集群的 `write` 已经回到 `object_store`，对象存储仍然是权威，删 PG 侧是安全的），
> 并清掉两台节点的 mirror store 让历史标记连同回填一起重来。
> 🔴 **不要只清 mirror store**：回填重放时 `begin` 会拿到 `AlreadyExists`，
> 那 32 行的错时刻会原样留下 —— 只是这次会被比对**查出来**，然后把判据卡住。
