...
- `crates/aenv-egress`: the runtime-to-broker contract for `network.rules` (identity header, framing, `BrokerTransport`, `Handler`, `CredentialSource`, `UpstreamGuard`) and the broker dispatch core. `aenv-node` links its `core` feature only; `make check-crate-boundaries` fails if it links `tls`, if any single feature stops building on its own, or if the crate gains a database, the byte half, `aenv-core` or a second TLS stack
