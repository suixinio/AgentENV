...
12. Run `make test-uffd` for the `uvm-uffd` handler suite. It creates its own userfaultfd with `UFFD_USER_MODE_ONLY`, which an unprivileged account may do under `vm.unprivileged_userfaultfd=0`, so it needs no host setup and no capability runner. With `AENV_MEMORY_SNAPSHOT_BACKEND=uffd AGENTENV_MEMORY_SNAPSHOT_TRACK_DIRTY_PAGES=true` in the environment, the integration suites of step 8 resume every snapshot through the daemon's userfaultfd server instead of a memory device. Firecracker creates that userfaultfd itself, which under `vm.unprivileged_userfaultfd=0` takes `CAP_SYS_PTRACE`; `scripts/run-with-capabilities.sh` grants it alongside `CAP_NET_ADMIN` and `CAP_SYS_ADMIN`, and a node running as root has it anyway; the daemon suite's `serve_memory_uffd_*` cases (`make test-ublk-daemon-nbd`) cover the daemon side without Firecracker by playing the VMM themselves.
