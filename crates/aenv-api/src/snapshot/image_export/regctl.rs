...
impl Regctl {
    ...
    pub async fn manifest_head(&self, image_ref: &str) -> RepositoryResult<Option<String>> {
    ...
    pub async fn manifest_put(&self, image_ref: &str, manifest: &Path) -> RepositoryResult<()> {
        self.put_file(
            &[
                "manifest",
                "put",
    ...
    pub async fn blob_head(&self, repository: &str, digest: &str) -> RepositoryResult<bool> {
...
pub mod fixture {
    ...
    pub fn external_ref(bytes: &[u8], repo_blob_url: &str) -> OverlaybdLayerRef {
    ...
    pub fn managed_ref(bytes: &[u8]) -> OverlaybdLayerRef {
    ...
    pub fn blob_path(dir: &Path, repository: &str, digest: &str) -> PathBuf {
        dir.join("state")
            .join("blobs")
            .join(repository)
            .join(digest)
    ...
    pub fn seed_blob(dir: &Path, repository: &str, digest: &str, bytes: &[u8]) {
    ...
    pub fn seed_manifest(dir: &Path, image_ref: &str, manifest_digest: &str) {
    ...
    pub fn stored_manifest(dir: &Path, image_ref: &str) -> (Vec<u8>, String) {
        let store = manifest_dir(dir, image_ref);
    ...
    pub fn argv_lines(dir: &Path) -> Vec<String> {
