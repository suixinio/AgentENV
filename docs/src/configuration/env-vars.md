...
| `GATEWAY_COLD_LOOKUP_TIMEOUT` | `3s` | Bounds the cold-path `LookupNode` call that a routing-projection miss and an undecided wake-up both fall through to — separate from `GATEWAY_REQUEST_TIMEOUT`, so a slow or unreachable target on this one path cannot hold a request open for gRPC's connection backoff. Sourced from the `cold-lookup-config` ConfigMap in `deploy/k8s/base`. |
