...
#[async_trait]
pub trait SourceImageStore: Send + Sync + std::fmt::Debug {
    async fn open(
        &self,
        manifest_digest: &str,
...
#[async_trait]
pub trait SourceImageEntry: Send {
...
#[derive(Clone)]
pub struct LocalImageServices {
    pub source_images: Arc<dyn SourceImageStore>,
    pub overlaybd_layers: Arc<dyn OverlaybdLayerStore>,
    pub runtime_refs: Arc<dyn RuntimeImageRefs>,
}
...
pub fn local_image_services_from_app_config(config: &AppConfig) -> LocalImageServices {
...
pub fn local_image_services_from_global_config() -> LocalImageServices {
...
#[cfg(test)]
pub fn test_local_image_services_from_service(
    cache: Arc<ImageCacheService>,
    gc_watermark: Option<(u64, u64)>,
    gc_min_age: Duration,
) -> LocalImageServices {
