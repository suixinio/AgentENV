...
| ~~`AENV_PAUSED_REGISTRY_DSN`~~ | — | Removed; never existed. The `postgres` backend reuses the process's own `[pg]` pool (see `AENV_PAUSED_REGISTRY_BACKEND` above) rather than taking a DSN of its own. **Setting it does nothing:** it is read by nothing and refused by nothing. The same is true of `AENV_PG_DSN`, another name that has never existed — `[pg]` is `Option<PgConfig>` and confique reaches a field from the environment only through `#[config(nested)]`, which may not be optional, so no key under `[pg]` can carry an `env =` binding at all. Supply `[pg].dsn` through the file `AENV_CONFIG_PATH` names or an `AENV_CONFIG_OVERLAY_PATH` overlay. |
...
| ~~`GATEWAY_COLD_LOOKUP_TIMEOUT`~~ | — | **Ignored since P4** of `docs/proposals/2026-09-01-client-proxy-api-alignment.md`: it bounded the cold-path `LookupNode` call a routing-projection miss and an undecided wake-up fell through to, and the gateway makes no such call any more — a miss goes to `apiproxy.ResumeSandbox`, and an api half that cannot be asked is a 502. The gateway's config loader reads no such key and refuses no such name. It stays declared in the `cold-lookup-config` ConfigMap in `deploy/k8s/base` for the rollback window (the previous gateway digest reads it) and is removed in the closing commit. |
| ~~`GATEWAY_QUERY_ONLY_SCHEDULER_ADDR`~~ | — | **Removed.** It named a second Scheduler-protocol endpoint that `LookupNode` alone would dial, for HA read traffic served by `services/scheduler`'s `--query-only` replica mode. That Go binary is deleted from the tree and the client-selection path it configured (`QueryOnlySchedulerClient`) went with it, so every `LookupNode` call goes to `GATEWAY_SCHEDULER_ADDR` like every other Scheduler RPC the gateway makes. **Setting it today does nothing:** the gateway's config loader reads no such key and refuses no such name. |
| ~~`GATEWAY_SCHEDULER_FALLBACK_DISABLED`~~ | — | **Removed.** It could skip the cold-path `LookupNode` call entirely, turning a projection miss into an immediate 503. Deleted outright rather than defaulted off — the call is unconditional again. **Setting it today does nothing.** |
| ~~`GATEWAY_SCHEDULER_FALLBACK_TIMEOUT`~~ | — | **Renamed to `GATEWAY_COLD_LOOKUP_TIMEOUT`** (above). The setting itself is unchanged and still load-bearing; only the name went, once the call it bounds stopped being a fallback to a process that might not be running. **Setting the old name today does nothing** — it is read by nothing and refused by nothing, and the cap silently stays at its `3s` default. |
