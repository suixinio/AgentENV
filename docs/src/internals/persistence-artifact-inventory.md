...
`persisted_sandbox_store_path` is a node-local scratch root, not a store. A pause captures the VM under it, stages the capture on the snapshot repository (`StagingPausePublisher`), and the api half commits the staged value into the snapshot catalog (`CommittingPausePublisher`); from then on the sandbox exists only as that catalog row (`source_kind = sandbox`, payload `PausedSandboxConfig`), with the same artifact layout as any other snapshot. No record of a paused sandbox is kept on a node, nothing under the scratch root survives a pause, and startup reclaim sweeps whatever a crash left behind.
...
- A paused sandbox is a committed snapshot row. Its bytes are repository artifacts like any other snapshot's; the capture scratch under `persisted_sandbox_store_path` is disposable.
