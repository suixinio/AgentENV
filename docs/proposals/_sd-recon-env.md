...
### 11.8 🔧 第五轮（2026-08-21，阶段 2c 上线当轮）：四条环境事实 ＋ 当前基线

#### ① 🔴 preStop 的射程 —— 已在 §7.2 B2 就地订正，这里只补它的**判据那一半**

§7.2 B2 记的是机理：preStop 的 drain 循环是 `while true` ＋ `sleep 3`，**没有超时也没有逃生口**，
而**任何一次 node Pod 删除都会跑它**（`deploy/k8s/base/agentenv-daemonset.yaml:163-183`）。
⇒ 判据一律照 `/v2/sandboxes` 数，必须读 **0**。

🔧 **补一条本轮才对上的**：**不要拿 `/nodes/{id}` 的 `sandboxCount` 当这条判据** ——
它只数 VM 还活着的那几个状态，暂停的记在**另一个字段** `sandboxPausedCount`
（`src/api/openapi.yml:1232` `:1250`，`NodeDetail` 侧 `:1290` `:1309`，两者在两个 schema 里**都是 required**：
`:1203` `:1208` / `:1261` `:1265`）。§11.3 的第二个坑说的就是这件事，
这里把它明确挂到 drain 判据上：**任何按 `sandboxCount` 单读设计的 drain 探针，期望值从一开始就是错的。**
🔴 这条对阶段 3 有直接后果 —— [`_sd-impl-phase3-role.md`](_sd-impl-phase3-role.md) §7.3 提的
preStop 改写正是「改读 `sandboxCount`」，那个方案要改成读两个字段之和，
否则把一个**已知会漏**的读法写进钩子里。已在该文 §15.5 就地订正。

#### ② 🔴 scheduler 副本数归零 ⇒ gateway 什么都路由不了 ⇒「中心不可达」探针必须走 node 直连

本轮要造「中心目录不可达」，最省事的做法是 `kubectl scale deploy/agentenv-scheduler --replicas=0`。
**它做得到不可达，但同时把 gateway 也废了** —— gateway 的路由要问 scheduler，
scheduler 没了，经 gateway 的那一发在到达任何 node 之前就失败，
于是探针测到的是「gateway 502」，不是「node 面对一个够不着的中心目录时怎么做」。

⇒ 🔴 **凡是要观察节点在中心不可达时行为的探针，一律走 node 直连**（`http://<node-ip>:8000`，
带 `x-agentenv-control-plane` ＋ `X-API-Key`），不要经 30800 的 gateway。
这与 §11.1 的两条「以为能用、其实是空转」是同一族错误：**手法废掉的是探针本身，而结果读起来像通过。**

#### ③ 🔧 快照删除：本轮之前**没有任何操作员路径**，集群上因此攒了三条孤儿

`/snapshots/{snapshotID}` 过去只有 `get`，`DELETE` 返回 **405**；
`SnapshotManager::delete` 只能作为「删沙箱」或「快照被取代」的**副作用**被触到。
⇒ 一台沙箱已经没了的快照，全仓没有任何 API 能删它。**dev 集群上有三条这样的孤儿。**

🔴 **剩下的那根杠杆是 `psql`，而它是错的那根**：只删 PG 的行，会留下对象存储持有一个数据库没有的快照 ——
正是 `admit_read_side` / `guard_read_side` 会据以**拒绝读侧切换**的那种总体分歧
（`src/snapshot/repository/mirror/population.rs`，模块头逐字）。

✅ **`86f1da6` 加了 `DELETE /snapshots/{snapshotID}`**（`src/api/openapi.yml:2056-2057`）：
admin 标签、admin 鉴权、走目录 ⇒ `write = "both"` 下两个 store 都掉行（够不着的那个进镜像队列），
别名与产物一并走；**按 `AnyStatus` 读**，所以没构建过的模板也删得掉；
名字没匹配上回 **404** 而不是客气的 204。
🔴 **它不接任何自动路径，也不问谁依赖这个快照** —— 一台暂停沙箱的快照就是那台沙箱唯一的耐久副本。

#### ④ catalog 集成套件会写进被指向的集群 —— 见 §11.7，本轮无新增

#### ⑤ 🔧 当前集群基线（2026-08-21，阶段 2c 上线后）

阶段 3 从这里开始，判据要按这份基线写：

| 项 | 值 |
|---|---|
| 运行中的一批 | `sd2c-*` |
| `snapshot.catalog.write` | **`both`**（双写） |
| `snapshot.catalog.read` | **`postgres`** |
| 目录总体 | 30 条 `p0-seed-*` ＋ 2 条模板 = **N = 32**（两侧一致） |
| `mirror_lag` / `mirror_diverged` | 两台 node 上**都是 0** |
| 三个路由投影开关 | **全 on**（§11.4 的终态） |
| F4 sweep 开关 | **off** |
| 镜像 store | **空** |

🔴 **「两个 gauge 都是 0」在这份基线里不是「一致」的证据** ——
它只是「队列里没有欠着的写」。总体一致的证据是**直接总体比对**（`admit_read_side`），
理由与实测在 §11.9。

### 11.9 🔴 `mirror_lag == 0` 读起来一模一样：备份跑过了，和从来没跑过

本轮定的一条**长期判据**，不只对阶段 2：

**现象。** 镜像回填在这套集群上约 **100 ms** 跑完。所以「切读侧之前先看两个 gauge 归零」
这条判据，在「回填跑过并结清」与「回填**根本没跑**」两种相位下给出**完全相同**的读数。
实测撞过两次，第二次比第一次更隐蔽：把 PG 清空但**没清节点本地的镜像 store**，
「历史已入队」那个标记还在盘上 ⇒ 回填立刻返回、什么都没入队、什么都没比对，
于是三个数字一致同意「两个目录一致」——
而当时 **PG 0 行、对象存储 32 行**（`src/snapshot/repository/mirror/population.rs` 模块头逐字记着这两次）。

🔴 **⇒ 判据钉在「直接总体比对」上，用 `mirror_repaired_total` 佐证，不用 `mirror_lag` 当主判据。**
`admit_read_side`（`population.rs:359`）问两个目录各自持有什么、**谁都不信**，
不一致就拒绝**并点名 id**。
**集群上已经证过它有分辨力**：PG 31 / 对象存储 32 时，四个 gauge 全读 0，
总体判据照样拒绝，并把缺的那个 id 说了出来 —— 这正是 §8 第 2 条要的那种「先把它顶起来一次」。

🔴 **两条射程边界，照实写**（`population.rs` 模块头自己列的三条里的两条）：
1. **它比的是身份，不是内容。** 两个 store 各有一行同 id 但 `created_at_ms` / 别名 / payload 不同，**它放行**。
2. **它是一个节点在一个瞬间的视角。** 🔴 **本产品里没有任何东西做集群级聚合** ——
   指标是**每节点**的，一个集群级的判据必须自己把各节点加起来，
   而**一台节点离场会把它那份欠账从总和里带走，不留痕迹**。
   ⇒ 凡是写「全集群 X 为 0」的判据，都要先说清楚**是谁在加这个和**，以及**加的时候有几台在报**。

### 11.10 🔧 一条测试手法上的教训：两个时间戳落在同一毫秒里，测试就**不可能失败**

本轮抓到一个断言：它要区分的两个时刻由两次紧邻的 `now_unix_ms()` 产生，
在快机器上**落在同一毫秒**，于是「区分开了」与「没区分开」得到同一个值 ——
测试恒绿，且绿得毫无信息。

🔴 **规矩：任何基于时间先后的断言，都必须由测试自己制造一个可靠大于时钟粒度的间隔**
（显式推进时钟，或注入两个确定的时间戳），**不能指望执行耗时把它们分开**。
这与 §8 第 2 条是同一条：**一个在任何相位都给同样答案的判据，不是判据。**
