...
- Two backends share one contract suite in each of `src/orchestrator/store/contract.rs` (in-memory / Redis metadata store) and `crates/aenv-api/src/binding_store/contract.rs` (in-memory / Redis binding store). After changing either backend, run the Redis target so both sides are exercised.
